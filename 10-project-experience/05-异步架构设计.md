...
[⬅️ 上一篇：消息通知系统](04-消息通知系统.md) | [🏠 返回目录](../README.md) | [下一篇：权限与安全设计 ➡️](06-权限与安全设计.md)
...
```mermaid
flowchart LR
    Kafka[[Kafka\nquestion-events]] -->|search-sync-group| C1["搜索同步消费者\n→ 写 ES"]
    Kafka -->|tag-count-group| C2["计数消费者\n→ 标签/圈子计数"]
    Kafka -->|notification-group| C3["通知消费者\n→ 给关注者发通知"]
    Kafka -->|activity-group| C4["积分消费者\n→ 用户活跃度"]
```
