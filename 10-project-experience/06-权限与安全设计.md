...
[⬅️ 上一篇：异步架构设计](05-异步架构设计.md) | [🏠 返回目录](../README.md) | [下一篇：性能优化与踩坑 ➡️](07-性能优化与踩坑.md)
...
```mermaid
flowchart TD
    Admin["🔴 系统管理员\nSYSTEM_ADMIN\n全站内容管理 + 用户管理 + 圈子管理"] -->|包含所有权限| Moderator
    Moderator["🟡 圈子版主\nCIRCLE_MODERATOR\n管理本圈子内容 + 普通用户权限"] -->|包含普通用户权限| User
    User["🟢 普通用户\nUSER\n发帖/回答/评论/互动"]
```
...
flowchart LR
    ...
    Service -->|读取 Header| Context["UserContextHolder\n线程本地变量"]
    Context --> AOP["@RequireRole 注解\nAOP 切面鉴权"]
```
