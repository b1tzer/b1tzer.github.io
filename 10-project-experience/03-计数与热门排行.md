...
[⬅️ 上一篇：全文搜索系统](02-全文搜索系统.md) | [🏠 返回目录](../README.md) | [下一篇：消息通知系统 ➡️](04-消息通知系统.md)
...
```mermaid
flowchart LR
    Event["用户行为事件\nKafka消息"] --> Consumer["热度计算消费者"]
    Consumer --> DB["从 MySQL 读取\n完整统计数据"]
    DB --> Calc["HeatScoreCalculator\n重新计算热度分"]
    Calc --> ZSet["更新 Redis ZSet\nZADD + ZREMRANGEBYRANK"]

    Timer["定时任务\n每小时全量重算"] --> ActiveQ["扫描近7天活跃问题"]
    ActiveQ --> Calc
```
