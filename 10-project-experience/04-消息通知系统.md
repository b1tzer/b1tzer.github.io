...
[⬅️ 上一篇：计数与热门排行](03-计数与热门排行.md) | [🏠 返回目录](../README.md) | [下一篇：异步架构设计 ➡️](05-异步架构设计.md)
...
```mermaid
flowchart TD
    NewEvent["新通知事件"] --> CheckMerge{"Redis SETNX\n5分钟内是否\n已有同类通知？"}
    CheckMerge -->|否，首次| Insert["INSERT 新通知记录\nmerge_count = 1"]
    CheckMerge -->|是，聚合| Update["UPDATE merge_count + 1\n更新通知文案"]
    Insert --> IncrUnread["Redis 未读数 +1"]
    Update --> CheckUnread{"5分钟内\n未读数是否\n已累加过？"}
