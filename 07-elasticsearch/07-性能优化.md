...
[⬅️ 上一篇：ES 集群架构与分片机制](06-集群架构与分片机制.md) | [🏠 返回目录](../README.md) | [下一篇：ES 数据一致性：MySQL 与 ES 同步方案 ➡️](08-数据一致性.md)
...
flowchart LR
    subgraph 写入优化手段
        A["bulk 批量写入\n减少网络往返次数"] --> Perf["写入性能提升\n5~10倍"]
        B["增大 refresh_interval\n默认1s → 30s或-1\n减少 segment 生成频率"] --> Perf
        C["增大 translog flush 间隔\n降低磁盘 IO"] --> Perf
        D["关闭副本\n写入完成后再开启"] --> Perf
    end
```
