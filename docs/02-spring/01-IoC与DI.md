...
>
> 💡 **补充**：AOP 代理对象的生成，底层是通过 `BeanPostProcessor`（Bean 后置处理器）实现的——容器在每个 Bean 初始化完成后，调用 `BeanPostProcessor.postProcessAfterInitialization()` 判断是否需要生成代理，需要则返回代理对象替换原始对象。`BeanPostProcessor` 是容器的扩展点，属于 AOP 的"幕后机制"，不是与 IoC/AOP 并列的核心概念。
