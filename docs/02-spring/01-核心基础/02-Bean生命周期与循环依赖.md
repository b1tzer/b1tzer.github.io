...
- **③ Aware 接口回调**
    ...
    - `BeanFactoryAware` → 注入基础容器 BeanFactory（仅提供核心 IoC 功能）
    - `ApplicationContextAware` → 注入企业级容器 ApplicationContext（BeanFactory 的子接口，提供事件、国际化等完整功能）
...
三级缓存结构：
┌──────────────────────────────────┬────────────────────────────────────────────┐
│ 一级缓存 singletonObjects         │ 完整的 Bean（已完成全部生命周期）             │
│ 二级缓存 earlySingletonObjects    │ 早期引用（可能是原始对象，也可能是 AOP 代理）  │
│ 三级缓存 singletonFactories       │ ObjectFactory（首次调用时生成早期引用）       │
└──────────────────────────────────┴────────────────────────────────────────────┘
```

```mermaid
flowchart LR
    A1["创建 A<br>放入三级缓存<br>(此时 A 是半成品)"] --> A2["注入 B<br>发现 B 未创建"]
    A2 --> B1["创建 B<br>放入三级缓存"]
    B1 --> B2["注入 A<br>从三级缓存获取 A 的早期引用<br>(通过 ObjectFactory 生成，可能是 AOP 代理)"]
    B2 --> B3["B 初始化完成<br>放入一级缓存"]
    B3 --> A3["A 完成 B 的注入<br>初始化完成，放入一级缓存"]
```

> **为什么需要三级缓存而不是两级**：
>
> 常见的说法是"二级缓存存原始对象会导致 AOP 失效"，这个说法其实不严谨——如果只有二级缓存，完全可以在 Bean 实例化后立即判断是否需要代理，把代理对象直接放入二级缓存，AOP 照样生效。
>
> 真正的原因是 **"懒加载 + 性能优化"**：判断一个 Bean 是否需要 AOP 代理要遍历所有切点做匹配，成本不低。Spring 不想在**每个** Bean 实例化后都立刻做这件事——绝大多数 Bean 根本不会触发循环依赖，白白浪费性能。
>
> 三级缓存的 `ObjectFactory` 把"生成早期引用（判断+创建代理）"的动作**延迟到真正有人来取时才执行**：
>
> - 没有循环依赖时（99% 的场景）：`ObjectFactory` 永远不会被调用，省掉了代理判断开销，Bean 直接走正常生命周期（在第⑥步才创建代理）。
> - 有循环依赖时：才调用 `ObjectFactory`，提前生成代理，并把结果存入二级缓存以避免重复生成。
...
**Q：为什么需要三级缓存，二级缓存不够吗？**
> 本质原因是**性能优化**：判断一个 Bean 是否需要 AOP 代理需要遍历切点做匹配，成本不低。三级缓存用 `ObjectFactory` 把"生成早期引用（含代理判断）"的动作延迟到真正有人来取时才执行——没有循环依赖时（绝大多数场景）工厂根本不会被调用，省掉了开销；有循环依赖时才调用工厂生成代理，并把结果存入二级缓存避免重复生成。
