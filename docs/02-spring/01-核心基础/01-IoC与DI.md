...
> **一句话记忆口诀**：IoC 容器的底层是 `DefaultListableBeanFactory`，元数据是 `BeanDefinition`，注入是按 `DependencyDescriptor` 解析（类型 → `@Qualifier`/`@Primary` → 泛型 → 名字 fallback）；Bean 实例化有三条路径（反射构造器 / `@Bean` 工厂方法 / `FactoryBean`）；Spring 6 默认禁用循环依赖自动解决，AOT 下 `BeanDefinition` 在构建期固化。
...
> 📖 循环依赖的三级缓存机制与解决方案详见 [Bean 生命周期与循环依赖](02-Bean生命周期与循环依赖.md)，本文不再重复。
...
!!! tip "ApplicationContext 只是个壳"
    我们平时说的"Spring 容器"指 `ApplicationContext`，但真正持有 Bean 的容器是它内部的 `DefaultListableBeanFactory`。`AnnotationConfigApplicationContext` 的 `getBean()` 最终都会委托到 `beanFactory.getBean()`，`beanFactory` 就是一个 `DefaultListableBeanFactory` 字段。

### 3.2 每个接口解决什么问题

| 接口 | 存在的理由 | 典型使用方 |
| :-- | :-- | :-- |
| `BeanFactory` | 最小可用容器，定义"按名/按类型取 Bean"的契约 | 所有容器使用方 |
| `HierarchicalBeanFactory` | 支持父子容器——子容器找不到时向上查找，但父容器看不到子 | Spring MVC：Root Context 装 Service/DAO，DispatcherServlet 的 WebApplicationContext 装 Controller |
| `ListableBeanFactory` | 支持批量操作（`getBeansOfType(Xxx.class)`），是 `@Autowired List<T>` 的基础 | `@Autowired Map<String, MessageSender>` 的实现依赖此接口 |
| `AutowireCapableBeanFactory` | 对**容器外**的对象（如手动 `new` 出来的）执行依赖注入 | Spring 与其他框架整合：`ctx.getAutowireCapableBeanFactory().autowireBean(obj)` |
| `ConfigurableBeanFactory` | 允许运行时注册单例、添加 `BeanPostProcessor` | 框架二次开发 |
...
| 实现 | 定位 | 典型来源 |
| :-- | :-- | :-- |
| `GenericBeanDefinition` | **解析期**使用的通用定义，可后改 parent | 注解扫描、XML 解析的初始产物 |
| `RootBeanDefinition` | **合并后**用于实例化的最终定义，无 parent | `getMergedLocalBeanDefinition` 的返回值 |
| `AnnotatedGenericBeanDefinition` | 基于注解的定义，携带 `AnnotationMetadata` | `@Configuration` 类、`@ComponentScan` 扫到的类 |

!!! note "合并（merge）是一个关键动作"
...
> 📖 `BeanPostProcessor` / `BeanFactoryPostProcessor` 的具体使用方式详见 [Spring 扩展点详解](04-Spring扩展点详解.md)，本文只强调它们作用的对象不同。
...
> 📖 `@Primary`、`@Qualifier`、`@Scope`、`@Lazy` 等注解的完整语义详见 [Spring 常用注解全解](08-Spring常用注解全解.md)，本文只讲它们**参与依赖解析的先后顺序**。
...
> 📖 Spring 6 / Boot 3 的完整新特性（AOT 流程、原生镜像构建步骤、观测性 API）详见 [Spring 6 / Boot 3 新特性深度解析](../05-进阶与调优/02-Spring6-Boot3新特性深度解析.md)。本文只关注"IoC 视角"的影响。
