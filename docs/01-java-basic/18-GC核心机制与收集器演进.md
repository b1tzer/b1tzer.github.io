...
title: GC 核心机制与收集器演进 —— 可达性分析 / 三色标记 / 写屏障 / 染色指针的物理透视
---

# GC 核心机制与收集器演进 —— 可达性分析 / 三色标记 / 写屏障 / 染色指针的物理透视

!!! info "**GC 核心机制与收集器演进 一句话口诀**"
    - **可达性分析 + GC Roots 六大来源**是 GC 的物理根基 —— 从 Roots 出发扫不到的就是垃圾；循环引用在可达性分析下天然不成问题（**引用计数才怕循环**，Python 用引用计数、Java 从 1.2 起就抛弃了它）。
    - **三色标记（白 / 灰 / 黑）** 是所有并发 GC 的公共语言；并发标记的漏标**只有两种解法** —— **CMS 走增量更新（Incremental Update，记脏 Card）** vs **G1 / ZGC 走 SATB（Snapshot At The Beginning，记旧值）**。**没有第三种**。
    - **写屏障是"业务线程留痕、GC 线程补扫"的物理契约**：JIT 在每条"引用字段赋值"字节码前后自动插入拦截代码，把原本需要"全堆 STW 扫描"的工作压缩到"只扫并发期间变脏的那一小部分" —— 本质上是**用全量扫描换增量扫描**，STW 从秒级压到毫秒级。
    - **三种 GC 算法的分代物理映射**：新生代用**复制**（死多活少、浪费一半也划算）、老年代用**标记-整理**或**标记-清除**（死少活多、复制成本太高） —— 这是弱分代假说的直接工程化。
    - **收集器演进主线一句话**：Serial → Parallel（吞吐）→ CMS（首次并发标记）→ G1（Region + 可预测停顿）→ ZGC（染色指针 + 亚毫秒）。每一代都在回答同一个问题：**还能把哪些必须 STW 的事挪到业务线程并发时做？**
    - **ZGC 染色指针把 GC 状态编在指针高位 4 位**（Finalizable / Remapped / Marked1 / Marked0，原版 ZGC · 分代 ZGC 位布局有所调整），配合**读屏障（Load Barrier）** 实现对象**并发转移** —— 这是 ZGC 亚毫秒停顿与堆大小无关的根本原因。

**你能立刻答上来吗？**

- GC 日志显示 `real=0.02s` 但 `Total time for which application threads were stopped: 5.2s`，剩下的 5 秒去哪了？为什么老手第一反应会指向 Safepoint 空洞？
...
- G1 的 RSet（Remembered Set）为什么让 G1 内存占用比 CMS 高？"记录哪些别的 Region 指向我"这件事的物理成本是什么？
...
> 📖 **边界声明**：本文聚焦"GC 算法机制与收集器实现原理"，以下主题请见对应姊妹文档：
>
> - 内存分区、对象头、Mark Word 五态多态、压缩指针 32GB 边界 → [12a JVM 内存分区与对象布局](@java-JVM-内存分区与对象布局)（本文的 GC 标记态 Mark Word 低 2 bit = `11` 视角是三处穿刺之一）
> - GC 调优参数、OOM 排查、`jcmd VM.native_memory` 完整链路、生产 checklist → [12c GC 调优实战与常见误区](@java-JVM-GC调优实战与常见误区)
> - 容器化 JVM、虚拟线程、JFR、**分代 ZGC**（JEP 439 / JEP 474）落地 → [12d JVM 现代实践与前沿技术](@java-JVM-现代实践与前沿技术)
> - CPU 级 Memory Barrier + happens-before + `synchronized` 锁升级完整链路 → [10a JMM 与线程同步](@java-并发-JMM与线程同步)（本文的 Write Barrier / Load Barrier 专指 GC 屏障，不重讲 JMM 内存屏障）
> - `Reference` 强度族的应用场景（`WeakHashMap` / `ThreadLocalMap.Entry`） → [08 集合框架](@java-数据结构-集合框架) §"WeakHashMap 与引用强度族"（本文提供"强 / 软 / 弱 / 虚 + `ReferenceQueue`" 家族源头，具体集合应用由 08 承接）
> - `Klass` / `oop` 二元模型完整展开 → [01 面向对象](@java-字节码-面向对象) §"对象头与 Klass Pointer"
...
**老手看完这份日志会秒级反应到一个词** —— **TTSP 空洞（Time-To-SafePoint gap）**。GC 日志里那个 `[Times: user=... sys=... real=...]` 段只统计"真正干活"的时间（`at safepoint`），而 `Total time for which application threads were stopped` 是**"业务被停"** 的总时长，两者差值就是"业务线程从收到 GC 通知到真正跑到 Safepoint 挂起"的等待时间（TTSP）。

**根本原因**：STW 停顿 = **TTSP（等所有线程到齐） + at safepoint（真正干活）**。GC 日志的 `real=0.02s` 只是后者，前者被隐藏在 `Total time` 里 —— 需要 `-Xlog:safepoint=info`（JDK 9+）或 `-XX:+PrintSafepointStatistics`（JDK 8）才能看得清。事故的直接肇因是某个批处理任务里有一段 `for (int i = 0; i < 200_000_000; i++) { ... }` 的 counted loop，HotSpot 默认**不在 counted loop 的回边插 Safepoint** —— 其他线程触发 GC 后，JVM 必须一直等到这个循环跑完（大约 5 秒）才能让整个进程进入 STW。

**顿悟点**：调 GC 不只是调算法本身，还要**关注所有业务线程能不能快速到达 Safepoint** —— 这才是所有并发 / 低延迟收集器的**真正前提**。JDK 10+ 可用 `-XX:+UseCountedLoopSafepoints` 在 counted loop 回边也插 Safepoint 关闭这个空洞。
...
**顿悟点**：CMS 的 Mark-Sweep 不移动对象，长期运行必定碎片化 —— 即使总剩余空间充足也可能因为**找不到连续空间**而无法分配大对象，被迫触发 Full GC。**这是 CMS 无法根治的两个硬伤（碎片 + 并发模式失败），也是 G1 用 Region + Compact 全面替代它的物理动机**。JDK 9 起 CMS 被标记为废弃、JDK 14 起彻底移除。

### 1.3 反问引子：老手也未必答得上的 6 个 GC 悬案

- **悬案 1**：GC Roots 到底有几种来源？除了"栈上局部变量"和"静态字段"，还有哪些？为什么 Python 的引用计数处理不了循环引用、而 Java 的可达性分析天然不成问题？
- **悬案 2**：三色标记的漏标为什么只有两种解法（增量更新 vs SATB）？没有第三种吗？为什么 CMS 选前者、G1 / ZGC 选后者？
- **悬案 3**：写屏障是"每次赋值都跑一段代码" —— 这个开销有多大？为什么 Parallel GC 吞吐最高（提示：它没有写屏障）？
- **悬案 4**：ZGC 的染色指针把 4 bit 编在指针高位 —— 那业务代码用这个"被染色"的指针访问对象时会不会读到脏数据？读屏障是怎么"透明修正"的？
- **悬案 5**：G1 的 RSet（Remembered Set）为什么让 G1 内存占用比 CMS 高？"记录哪些别的 Region 指向我"这件事的物理成本是什么？
- **悬案 6**：逃逸分析 + 标量替换到底把对象分配到栈上了吗？为什么《深入理解 JVM》早期版本的"栈上分配"说法是误传？

这六个悬案的答案都写在 HotSpot 源码 `hotspot/share/gc/*` 目录 + `-Xlog:gc*,safepoint` 日志里。
...
- **痛点 C**：ZGC 承诺"堆再大 STW 都 < 1ms"到底怎么做到 → 第二层 §2.3 揭染色指针位布局 + 第三层 §3.7 揭读屏障并发转移物理链

---

## 2. 第二层：字节码考古 —— `-Xlog:gc*+safepoint` + `hotspot/share/gc/*` + JIT 屏障代码的三件套穿刺
...
### 2.1 `-Xlog:safepoint=info` 揭 TTSP 空洞 —— 老手也没看过的 GC 日志隐藏项
...
[3.421s][info][safepoint]   At safepoint:       21.4 ms         ← at safepoint（真正干活）
[3.421s][info][safepoint]   Total:              5213.7 ms       ← STW 总时长
```

**顿悟点**：GC 日志里 `[Times: user=... sys=... real=...]` 段只统计 `At safepoint`；`Reaching safepoint` 才是"业务线程被卡住等 GC 开始"的真实等待时间。TTSP 飙高说明有线程迟迟不到达 Safepoint —— 高频命中 counted loop 空洞（回边不插 Safepoint、必须等循环跑完）。

### 2.2 `hotspot/share/gc/g1/g1CollectedHeap.cpp` 源码考古 —— G1 Region 化的物理入口
...
- 每个收集器都是 `CollectedHeap` 的一个 C++ 子类 —— 从设计模式看是**策略模式的极致落地**
- `HeapRegionManager` 是 G1 的物理骨架 —— 堆不再是"新生代 + 老年代"两块连续内存，而是 1024+ 个等大 Region
...
- **写屏障成本**：每条引用赋值多 2~4 条机器指令 —— 引用密集业务（大图遍历、ORM、大量 setter）**开销可达 5%~10%**。这就是"Parallel 吞吐最高"的物理根源（它没有写屏障）
...
## 3. 第三层：物理内存布局 —— GC Roots + 三色标记 + 五大收集器完整视图

### 3.1 可达性分析与 GC Roots 六大来源（第一张核心物理图）
...
### 3.2 三色标记算法与漏标的两种解法（第二张核心物理图 · 全站首发源头）
...
**顿悟点**：**漏标的解法只有两种、没有第三种** —— 要么"发现黑色新增引用 → 把黑改灰重扫"，要么"发现灰色删除引用 → 保守视为存活到下轮"。**其余方案都是这两条主线的变体**。

### 3.3 写屏障与 Card Table 的物理协作图（第三张核心物理图）
...
!!! note "📖 术语家族：`*Barrier` 屏障族"
    ...
    **易混点**：GC Write / Load Barrier 与 JMM Memory Barrier 完全是两回事 —— 前者由 GC 逻辑决定"记什么"，后者由 CPU 决定"重排/可见性" → 📖 完整 JMM 视角详见 [10a JMM 与线程同步](@java-并发-JMM与线程同步)

    **读屏障是 ZGC 的标志性技术** —— 让"对象转移"也能并发完成，代价是每次读引用多几条指令

### 3.4 三种 GC 算法的分代物理映射（第四张核心物理图）
...
**顿悟点**：CMS 的历史意义（**首次把标记 / 清除挪到并发**）与历史局限（**碎片 + 并发模式失败不可根治**）是同一枚硬币两面 —— 这也是 JDK 9 废弃 / JDK 14 移除的物理原因。

### 3.6 G1 的 Region + RSet + Mixed GC（第六张核心物理图）

**Region 化堆布局**（G1 的物理骨架）：
...
### 3.7 ZGC 染色指针 + 读屏障并发转移（第七张核心物理图 · ZGC 亚毫秒的物理根源）
...
> 📖 分代 ZGC（JEP 439 JDK 21 引入、JEP 474 JDK 23 默认）位布局有所调整（Young / Old 位取代原 Marked0/1）→ 完整收益对比见 [12d JVM 现代实践与前沿技术](@java-JVM-现代实践与前沿技术) §"分代 ZGC"

### 3.8 Safepoint / Safe Region 与 STW 的物理契约（第八张核心物理图）
...
**顿悟点**：GC 日志的 STW 停顿 = **TTSP（等线程到齐） + at safepoint（干活）** —— counted loop 空洞是 TTSP 高的头号嫌疑。

### 3.9 逃逸分析 + 标量替换（第九张核心物理图 · 减小 GC 压力的第一道防线）
...
!!! note "📖 术语家族：`*Reference` 引用强度族"
    ...
    **家族成员**：
    ...
    | `SoftReference<T>` | 内存不足时回收（Full GC 前最后救命） | 内存敏感缓存（图片缓存） | `java.lang.ref.SoftReference` |
    ...
    **一句话总结**：**软引用防 OOM、弱引用防泄漏、虚引用做清理通知**
    ...
    📖 具体集合应用（`WeakHashMap` / `ThreadLocalMap.Entry`）见 [08 集合框架](@java-数据结构-集合框架) §"WeakHashMap 与引用强度族"
...
**物理根源**：HotSpot 默认不在 `for (int i = 0; i < N; i++)` 的回边插 Safepoint → 循环体极长时其他线程触发 GC 会一直等 → TTSP 飙到秒级（就是 §1.1 事故来源）。
...
**物理根源**：CMS 内存碎片 + 并发模式失败**不可根治** —— 一次退化到 Serial Old 就是十几秒 STW（就是 §1.2 事故来源）。
...
**物理根源**：Parallel 没有写屏障 → 吞吐最高 → 但全程 STW，一次 GC 可达秒级。
...
**物理根源**：三种引用的 GC 回收时机不同 —— Soft 只在 Full GC 前救命回收、Weak 每次 GC 必回收、Phantom `get()` 恒返回 null。
...
**物理根源**：标量替换是**减小 GC 压力的第一道防线**；大量短命对象若能被拆散到局部变量、Eden 分配速率显著下降、Young GC 频率随之降低。
...
| 来源篇 | 伏笔内容 | 本文落地位置 | 状态 |
| :-- | :-- | :-- | :-- |
| **12a 内存分区与对象布局** | Mark Word GC 标记态（低 2 bit = `11`）+ forwarding pointer —— `12b` 需承接三色标记算法中 Mark Word 的具体使用（★★★★★） | §3.2 三色标记状态机 + §3.7 ZGC 染色指针（对象转移时 forwarding pointer 的使用） | ✅ 闭环 |
| **08 集合框架** | `*Reference` 强度族完整介绍 —— `12b` 需承接完整回收链（★★★） | §3.9 `*Reference` 引用强度族术语家族卡片 | ✅ 闭环 |
| **00 综览页** | 三色标记 + 写屏障是所有并发 GC 的公共语言 —— `12b` 需在 §"三色标记" 完整穿刺两种漏标解法（★★★★） | §3.2 三色标记算法 + §3.3 写屏障 + 术语家族卡片`*Barrier` 屏障族 | ✅ 闭环 |

### 5.2 本文埋下的伏笔

| 本篇 → 目标篇 | 伏笔内容 | 优先级 |
| :-- | :-- | :-- |
| `12b GC 核心机制` → [12c GC 调优实战](@java-JVM-GC调优实战与常见误区) | 写屏障代价 5%~10%、CMS 三大缺陷、G1 RSet 内存开销、ZGC 读屏障额外开销 —— `12c` 需承接完整 GC 参数调优链路 + Full GC 排查 checklist + G1 vs ZGC 生产选型 | ★★★★★ |
| `12b GC 核心机制` → [12d JVM 现代实践](@java-JVM-现代实践与前沿技术) | 分代 ZGC（JEP 439 / JEP 474）位布局调整 + 与原版 ZGC 收益对比 + Loom 虚拟线程栈扫描 —— `12d` 需承接前沿场景 | ★★★★★ |
| `12b GC 核心机制` → [13 NIO 与 IO 模型](@java-IO-NIO与IO模型深度解析) | 直接内存 GC 回收路径 · Cleaner 机制 · Netty ByteBuf 泄漏排查 —— `13` 需承接零拷贝的 GC 视角 | ★★★ |
| `12b GC 核心机制` → [12a 内存分区与对象布局](@java-JVM-内存分区与对象布局) | Mark Word 五态的 GC 标记态视角 + 三色标记与低 2 bit 物理映射 —— 已在 §3.2 完整闭环 | ✅ 已闭环 |
| `12b GC 核心机制` → [10a JMM 与线程同步](@java-并发-JMM与线程同步) | Memory Barrier CPU 级视角 —— `10a` 已完整承接 happens-before + volatile / synchronized 内存屏障 | ✅ 已闭环 |
| `12b GC 核心机制` → [08 集合框架](@java-数据结构-集合框架) | `*Reference` 引用强度族源头卡片 —— 已在 §3.9 完整承接 | ✅ 已闭环 |

### 5.3 Q&A（源码机制题速答）

**Q1**：为什么年轻代和老年代要分开？弱分代假说的物理根源是什么？

> **答**：**90% 对象朝生夕死**（弱分代假说） · Minor GC 只扫新生代（约 1/3 堆）而非全堆，成本压缩 3 倍以上 · 新生代死多活少用**复制算法**（浪费一半空间无所谓）、老年代死少活多用**标记-整理**（复制成本太高）—— 这就是分代收集的全部物理根源。

**Q2**：G1 是怎么做到可预测停顿的？RSet 起什么作用？

> **答**：Region 化 + `-XX:MaxGCPauseMillis` 目标 + **选垃圾最多的部分 Region 回收**（Garbage First）→ 在有限时间回收最多垃圾 · **RSet 让 Region 独立回收**（只扫本 Region 的 RSet 而非全堆），代价是每次引用写入都要更新 RSet → G1 内存占用比 CMS 高 5%~10%。

**Q3**：ZGC 为什么停顿时间与堆大小无关、始终 < 1ms？

> **答**：**染色指针 4 位编码 GC 状态**让元数据与地址同槽位复用 + **读屏障 Self-Healing** 让对象转移可并发（业务线程读到旧指针时自己修正）+ **STW 阶段只剩标记 GC Roots** —— 三者合起来让"必须停顿"的工作与堆大小解耦。

**Q4**：三色标记的漏标问题，CMS 和 G1 分别用什么方案解决？为什么只有这两种？

> **答**：**CMS 用增量更新**（Post-write barrier · 记脏 Card · 黑对象新增引用时把它重标为灰）· **G1 / ZGC 用 SATB**（Pre-write barrier · 记旧值 · 灰对象删除引用时保守视为存活）· **漏标的物理根源要么"黑对象新增指向白" 要么"灰对象删除对白的引用"—— 两条链路只能各选其一**，其他方案都是这两条主线的变体。

**Q5**：Soft / Weak / Phantom Reference 到底什么时候会被回收？分别用于什么场景？

> **答**：**Soft** 在 Full GC 前救命回收（`-XX:SoftRefLRUPolicyMSPerMB` 控制存活时长）· **Weak** 下次 GC 必定回收 · **Phantom** `get()` 恒返回 null、只做通知 · 场景对应"**软防 OOM、弱防泄漏、虚做清理**"三句口诀。

> 📖 **写屏障代价量化、Full GC 排查、G1 vs ZGC 生产选型、分代 ZGC 收益**已在 [§2.3 折叠块](#23-jit-屏障机器码考古--写屏障--读屏障的真实成本) / [12c GC 调优实战](@java-JVM-GC调优实战与常见误区) / [12d JVM 现代实践](@java-JVM-现代实践与前沿技术) 给出答案，本文不再重复，专注"GC 算法与收集器实现原理"题。
