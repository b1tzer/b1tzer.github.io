...
>
> - 内存分区、对象头、Mark Word 五态多态、压缩指针 32GB 边界 → [JVM 内存分区与对象布局](@java-JVM-内存分区与对象布局)（本文的 GC 标记态 Mark Word 低 2 bit = `11` 视角是三处穿刺之一）
> - GC 调优参数、OOM 排查、`jcmd VM.native_memory` 完整链路、生产 checklist → [GC 调优实战与常见误区](@java-JVM-GC调优实战与常见误区)
> - 容器化 JVM、虚拟线程、JFR、**分代 ZGC**（JEP 439 / JEP 474）落地 → [JVM 现代实践与前沿技术](@java-JVM-现代实践与前沿技术)
> - CPU 级 Memory Barrier + happens-before + `synchronized` 锁升级完整链路 → [并发基础：JMM 与线程同步](@java-并发-JMM与线程同步)（本文的 Write Barrier / Load Barrier 专指 GC 屏障，不重讲 JMM 内存屏障）
> - `Reference` 强度族的应用场景（`WeakHashMap` / `ThreadLocalMap.Entry`） → [集合框架](@java-数据结构-集合框架) §"WeakHashMap 与引用强度族"（本文提供"强 / 软 / 弱 / 虚 + `ReferenceQueue`" 家族源头，具体集合应用由 08 承接）
> - `Klass` / `oop` 二元模型完整展开 → [面向对象](@java-字节码-面向对象) §"对象头与 Klass Pointer"
...
| 本篇 → 目标篇 | 伏笔内容 | 优先级 |
| :-- | :-- | :-- |
| `12b GC 核心机制` → [GC 调优实战与常见误区](@java-JVM-GC调优实战与常见误区) | 写屏障代价 5%~10%、CMS 三大缺陷、G1 RSet 内存开销、ZGC 读屏障额外开销 —— `12c` 需承接完整 GC 参数调优链路 + Full GC 排查 checklist + G1 vs ZGC 生产选型 | ★★★★★ |
| `12b GC 核心机制` → [JVM 现代实践与前沿技术](@java-JVM-现代实践与前沿技术) | 分代 ZGC（JEP 439 / JEP 474）位布局调整 + 与原版 ZGC 收益对比 + Loom 虚拟线程栈扫描 —— `12d` 需承接前沿场景 | ★★★★★ |
| `12b GC 核心机制` → [Java NIO 与 I/O 模型](@java-OS-NIO与IO模型) | 直接内存 GC 回收路径 · Cleaner 机制 · Netty ByteBuf 泄漏排查 —— `13` 需承接零拷贝的 GC 视角 | ★★★ |
| `12b GC 核心机制` → [JVM 内存分区与对象布局](@java-JVM-内存分区与对象布局) | Mark Word 五态的 GC 标记态视角 + 三色标记与低 2 bit 底层映射 —— 已在 §3.2 完整闭环 | ✅ 已闭环 |
| `12b GC 核心机制` → [并发基础：JMM 与线程同步](@java-并发-JMM与线程同步) | Memory Barrier CPU 级视角 —— `10a` 已完整承接 happens-before + volatile / synchronized 内存屏障 | ✅ 已闭环 |
| `12b GC 核心机制` → [集合框架](@java-数据结构-集合框架) | `*Reference` 引用强度族源头卡片 —— 已在 §3.9 完整承接 | ✅ 已闭环 |
...
> 📖 **写屏障代价量化、Full GC 排查、G1 vs ZGC 生产选型、分代 ZGC 收益**已在 [§2.3 折叠块](#23-jit-屏障机器码考古--写屏障--读屏障的真实成本) / [GC 调优实战与常见误区](@java-JVM-GC调优实战与常见误区) / [JVM 现代实践与前沿技术](@java-JVM-现代实践与前沿技术) 给出答案，本文不再重复，专注"GC 算法与收集器实现原理"题。
