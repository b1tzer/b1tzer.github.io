...
>
> - 内存分区、对象头、Mark Word 五态多态、压缩指针 32GB 边界 → [JVM 内存分区与对象布局](@java-JVM-内存分区与对象布局)（本文的 GC 标记态 Mark Word 低 2 bit = `11` 视角是三处透视之一）
...
| **00 综览页** | 三色标记 + 写屏障是所有并发 GC 的公共语言 —— `12b` 需在 §"三色标记" 完整透视两种漏标解法（★★★★） | §3.2 三色标记算法 + §3.3 写屏障 + 术语家族卡片`*Barrier` 屏障族 | ✅ 闭环 |
...
> **答**：**Soft** 在 Full GC 前回收（`-XX:SoftRefLRUPolicyMSPerMB` 控制存活时长）· **Weak** 下次 GC 必定回收 · **Phantom** `get()` 恒返回 null、只做通知 · 场景对应"**软防 OOM、弱防泄漏、虚做清理**"三句口诀。
