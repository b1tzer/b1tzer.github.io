...
!!! info "**GC 核心机制与收集器演进 一句话口诀**"
    - **可达性分析 + GC Roots 六大来源**是 GC 的基础 —— 从 Roots 出发扫不到的就是垃圾；循环引用在可达性分析下天然不成问题（**引用计数才怕循环**，Python 用引用计数、Java 从 1.2 起就抛弃了它）。
    ...
    - **写屏障是"业务线程留痕、GC 线程补扫"的底层契约**：JIT 在每条"引用字段赋值"字节码前后自动插入拦截代码，把原本需要"全堆 STW 扫描"的工作压缩到"只扫并发期间变脏的那一小部分" —— 本质上是**用全量扫描换增量扫描**，STW 从秒级压到毫秒级。
    - **三种 GC 算法的分代映射**：新生代用**复制**（死多活少、浪费一半也划算）、老年代用**标记-整理**或**标记-清除**（死少活多、复制成本太高） —— 这是弱分代假说的直接工程化。
...
**顿悟点**：CMS 的 Mark-Sweep 不移动对象，长期运行必定碎片化 —— 即使总剩余空间充足也可能因为**找不到连续空间**而无法分配大对象，被迫触发 Full GC。**这是 CMS 无法根治的两个硬伤（碎片 + 并发模式失败），也是 G1 用 Region + Compact 全面替代它的根本动机**。JDK 9 起 CMS 被标记为废弃、JDK 14 起彻底移除。
...
- **悬案 5**：G1 的 RSet（Remembered Set）为什么让 G1 内存占用比 CMS 高？"记录哪些别的 Region 指向我"这件事的底层成本是什么？
...
**顿悟点**：CMS 的历史意义（**首次把标记 / 清除挪到并发**）与历史局限（**碎片 + 并发模式失败不可根治**）是同一枚硬币两面 —— 这也是 JDK 9 废弃 / JDK 14 移除的根本原因。
...
**根本原因**：三种引用的 GC 回收时机不同 —— Soft 只在 Full GC 前救命回收、Weak 每次 GC 必回收、Phantom `get()` 恒返回 null。
...
| `12b GC 核心机制` → [12a 内存分区与对象布局](@java-JVM-内存分区与对象布局) | Mark Word 五态的 GC 标记态视角 + 三色标记与低 2 bit 底层映射 —— 已在 §3.2 完整闭环 | ✅ 已闭环 |
...
**Q1**：为什么年轻代和老年代要分开？弱分代假说的根本原因是什么？

> **答**：**90% 对象朝生夕死**（弱分代假说） · Minor GC 只扫新生代（约 1/3 堆）而非全堆，成本压缩 3 倍以上 · 新生代死多活少用**复制算法**（浪费一半空间无所谓）、老年代死少活多用**标记-整理**（复制成本太高）—— 这就是分代收集的全部根本原因。
...
> **答**：**CMS 用增量更新**（Post-write barrier · 记脏 Card · 黑对象新增引用时把它重标为灰）· **G1 / ZGC 用 SATB**（Pre-write barrier · 记旧值 · 灰对象删除引用时保守视为存活）· **漏标的根本原因要么"黑对象新增指向白" 要么"灰对象删除对白的引用"—— 两条链路只能各选其一**，其他方案都是这两条主线的变体。
