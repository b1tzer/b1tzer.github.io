...
- ✅ 回收 [Java8 其他新特性](@java-番外-Java8其他新特性) 关联的知识点：**"Java 9 接口 `private` 方法是 Java 8 `default` 方法的自然演化 —— `91` §Java 9 语法新增承接"**（★★★）
...
**跨篇章关联说明**：番外附录以**"建立知识关联为主 · 少量展开"**为定位，所有深度分析任务下放给深度源码型 8 篇（`01`~`07` + `12b`~`12d`）。本篇的四条埋点会在各自深度源码型中被显式承接、构成"番外语法糖 ↔ 深度源码"的完整闭环。
