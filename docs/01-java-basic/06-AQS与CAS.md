...
`ReentrantLock`、`CountDownLatch`、`Semaphore`、`ReentrantReadWriteLock`、`CyclicBarrier` 等几乎所有并发工具类都基于 AQS 实现。
...
tryOptimisticRead()
  └─ 返回 stamp（当前版本号，奇数=有写锁，偶数=无写锁）

读取数据（不加锁）

validate(stamp)
  ├─ stamp 未变化 → 读取有效，直接使用 ✅
  └─ stamp 已变化（有写操作发生）→ 读取无效，升级为悲观读锁 🔄
```
...
:::warning
    **死锁排查**：当系统出现线程卡死时，使用 `jstack <pid>` 生成线程转储，搜索 `BLOCKED` 状态的线程和 `waiting to lock` 关键字。JDK 自带的死锁检测会在 jstack 输出末尾打印 `Found one Java-level deadlock` 提示。
