...
> 3. **内存与层高易调**：通过调整概率 p 与最大层数，能在内存和查找层数间灵活权衡
>
> 💡 **补充**：Redis ZSet 的完整底层是 **“skiplist + hash” 双结构**——skiplist 保障范围和有序操作高效，hash 表负责 O(1) 的 `ZSCORE member → score` 查找。
...
Kafka 的延迟操作（如延迟生产、延迟拉取）使用**分层时间轮 + `DelayQueue`** 的组合：时间轮负责 O(1) 的任务插入和到期触发，`DelayQueue`（底层最小堆）管理“有任务的 bucket”，让时间指针能跳过空 bucket、避免空转。
---
...
> 📌 约定：`(end)` 代表到达此节点即为一个完整单词（如 `app`/`apple`/`application`/`banana` 各有一个 `(end)` 标记）。
