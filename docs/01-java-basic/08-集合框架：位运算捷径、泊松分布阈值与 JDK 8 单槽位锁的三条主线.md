...
!!! note "ArrayList 为什么是 1.5 倍扩容？"
    **结论在先**：1.5 倍是在“扩容频率 vs 内存浪费”之间的**工程经验值**，没有严格的数学最优解。
...
!!! tip "HashMap 扩容时的 rehash 优化"
    **JDK 8 扩容时的 rehash 优化**：
    ...
    🚀 **性能要点**：扩容的总时间复杂度仍是 O(n)（需要转移所有节点），但**相对于 JDK 7 需要对每个 key 重新 `hash & (newN-1)` 的写法，省下了每个节点的 hash 计算与按位与开销**。
...
| `ArrayBlockingQueue` | 数组 + ReentrantLock（单锁） | **有界**生产消费，内存可控 |
...
**JDK 7**：分段锁（Segment），将数组分成默认 16 段，每段一把 `ReentrantLock`
**JDK 8**：**CAS + synchronized**，锁粒度细化到单个数组槽位；`synchronized` 锁的是该槽位的头节点（链表头 或 红黑树根）
