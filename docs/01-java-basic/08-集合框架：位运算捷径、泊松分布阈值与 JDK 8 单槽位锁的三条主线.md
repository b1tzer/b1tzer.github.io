...
!!! info "**集合框架一句话口诀**"
    ...
    - **HashMap 容量必须是 2^n，是 `hash & (n-1)` 位运算捷径的硬性绑定**：`(n-1)` 二进制全 1 才能让 `&` 等价于 `%`；`tableSizeFor` 就是"传入 10 → 返回 16、传入 17 → 返回 32"这套向上取整到 2^n 的位运算魔法。JDK 8 扩容时 `hash & oldCap` 只判断"新增那一位 bit 是 0 还是 1"，从 O(位数) 简化为 O(1) 判断——这三条位运算捷径共享同一条设计哲学。
