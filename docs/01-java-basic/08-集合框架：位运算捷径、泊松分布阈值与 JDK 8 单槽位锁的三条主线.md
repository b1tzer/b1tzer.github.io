...
- **悬案 1**：`new HashMap<>(17)` 底层实际把容量设为了几？为什么？—— 老手都答"16"或"32"，但很少能说清 `tableSizeFor` 那一串 `n |= n >>> 1;` 位运算的**根本动机**。
- **悬案 2**：JDK 8 HashMap 扩容为什么可以"不重新算 hash"？—— 老手都听过"高低位判断"，但很少能画出"`hash & oldCap` 只多用一位 bit"的底层链路。
- **悬案 3**：树化阈值 8 / 退化阈值 6 / `MIN_TREEIFY_CAPACITY = 64` —— 老手能背出这三个数字，但答得出"泊松分布 λ=0.5 下链长 8 的概率是 6×10⁻⁸"根本动机的极少。
- **悬案 4**：JDK 8 的 `ConcurrentHashMap` 把 JDK 7 的 `Segment` 分段锁**改回** `synchronized`——这是历史倒退吗？直接到底靠什么让 `synchronized` 反而更快？
