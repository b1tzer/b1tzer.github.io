---
doc_id: java-Java9-17新特性
title: Java 9-17 关键新特性
---
...
// 子类必须标注为 final / sealed / non-sealed 之一
public record Circle(double radius) implements Shape {}
public record Rectangle(double width, double height) implements Shape {}
public record Triangle(double base, double height) implements Shape {}

// 配合模式匹配 switch 表达式使用（Java 21 正式；Java 17 在 --enable-preview 下可用）
...
> 📌 **版本提示**：`switch` 的 `case Type t ->` 模式匹配与密封类穷举是在 **Java 21 才正式落地**（JEP 441）；Java 17 中需通过 `--enable-preview` 开启预览特性。若项目还在 Java 17 运行时或不想用预览特性，可用古典写法的 `instanceof` 模式匹配（见 §6）配合 `if-else` 达到类似效果。
...
### ❌ 坅3：Switch 表达式的 yield 关键字

```java
// ⚠️ 注意：Switch 表达式的块中必须用 yield 给出表达式值，
//      return 的语义是“从外层方法返回”，而不是从 switch 返回值
public String describe(int status) {
    String result = switch (status) {
        case 1 -> "活跃";
        case 2 -> {
            String msg = "已禁用";
            return msg; // ⚠ 合法编译，但语义是直接从 describe() 方法返回，
                        //   result 根本不会被赋值——多半是编写者想要的错误值语义
        }
        default -> "未知";
    };
    return result;
...
// ✅ 正确：用 yield 代表 switch 表达式的“当前分支结果值”
String result = switch (status) {
    case 1 -> "活跃";
    case 2 -> {
        String msg = "已禁用";
        yield msg; // yield 将 msg 给出为分支结果，继续往 result 赋值
    }
    default -> "未知";
};
```
