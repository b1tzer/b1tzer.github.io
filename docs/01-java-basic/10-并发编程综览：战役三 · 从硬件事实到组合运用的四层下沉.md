...
>
> ⚠️ **不可跳读**：`10b` 假定读者已读 `10a`；`10c` 假定已读 `10b`；`10d` 假定已读 `10a/10b/10c`。中途跳过一层，后面全部读不懂——这不是修辞，是并发这门学问本身就无法回避的四层同时穿透。
...
**顿悟点**：并发编程学习曲线陡峭的**根本原因**是"三层同时穿刺"——硬件层（CPU 指令）+ JVM 层（JMM 抽象）+ 应用层（JUC API），必须**自下而上**打通才能理解。任何跳过硬件层直接看 `ReentrantLock` 源码的尝试，最终都会卡在"`CAS` 到底是不是原子的、为什么 `park/unpark` 不用担心信号丢失、`ConcurrentHashMap.sizeCtl` 为什么用 CAS 而不是 `synchronized`"这类底层问题上——因为这些"为什么"的答案**全部躺在硬件层**。
...
- **`10 → 10d`**：本页 §3 提到 `ConcurrentHashMap.put` **同时穿透四层**——`10d` 需在"完整源码"章节承接"CAS 无锁插入 + `synchronized` 头节点锁 + `sizeCtl` CAS 争抢 + `ForwardingNode` 协助扩容"这条**四层穿透的底层链路**。
