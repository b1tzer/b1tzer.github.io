---
doc_id: java-并发-并发编程
title: 并发编程综览 —— 第三部分 · 从硬件事实到组合运用的四层下沉
---

# 并发编程综览：第三部分 · 从硬件事实到组合运用的四层下沉
...
**四次跃迁的共同规律**：每次跃迁都**不是把上一代废弃**，而是把上一代当作"更底层的原语"继续使用——`ReentrantLock` 没有取代 `synchronized`（JDK 6 之后二者性能持平）、`LongAdder` 没有取代 `AtomicLong`（低并发下前者反而更重）、虚拟线程也没有取代 `ForkJoinPool`（虚拟线程调度的载体线程池本身就是 `ForkJoinPool`）。这就是第三部分的核心节奏——**新工具是老工具的组合运用，不是替代**。
...
**关键结论**：并发编程学习曲线陡峭的**根本原因**是"三层同时交织"——硬件层（CPU 指令）+ JVM 层（JMM 抽象）+ 应用层（JUC API），必须**自下而上**打通才能理解。任何跳过硬件层直接看 `ReentrantLock` 源码的尝试，最终都会卡在"`CAS` 到底是不是原子的、为什么 `park/unpark` 不用担心信号丢失、`ConcurrentHashMap.sizeCtl` 为什么用 CAS 而不是 `synchronized`"这类底层问题上——因为这些"为什么"的答案**全部躺在硬件层**。

---

## 3. 第三部分内部认知下沉图：四层强耦合动线
