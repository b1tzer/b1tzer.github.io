...
!!! info "**战役三 · 并发全景一句话口诀**"
    ...
    - **JDK 21 虚拟线程改变了"什么时候用线程池"的答案，但没改变 AQS / CAS 的底层机制**——虚拟线程让"高并发不再依赖线程池"，但同步机制（`synchronized` / `ReentrantLock` / AQS）依然是虚拟线程的基石；`synchronized` 在虚拟线程中会 pin 到载体线程的排查思路详见 [12d JVM 现代实践](@java-JVM-现代实践与前沿技术)，本战役不承接。
