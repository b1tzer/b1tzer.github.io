...
>
> ⚠️ **不可跳读**：`10b` 假定读者已读 `10a`；`10c` 假定已读 `10b`；`10d` 假定已读 `10a/10b/10c`。中途跳过一层，后面全部读不懂——这不是修辞，是并发这门学问本身就无法回避的四层同时贯穿。
...
!!! info "**战役三 · 并发全景一句话口诀**"
    ...
    - **JDK 21 虚拟线程改变了"什么时候用线程池"的答案，但没改变 AQS / CAS 的底层机制**——虚拟线程让"高并发不再依赖线程池"，但同步机制（`synchronized` / `ReentrantLock` / AQS）依然是虚拟线程的基石；`synchronized` 在虚拟线程中会 pin 到载体线程的排查思路详见 [JVM 现代实践与前沿技术](@java-JVM-现代实践与前沿技术)，本战役不承接。
...
!!! note "📖 姊妹文档边界"
    ...
    **本页不讲**（点到即止 + `@doc_id` 下钻）：
    ...
    - JMM 四种内存屏障 + 锁升级完整机制 → [并发基础：JMM 与线程同步](@java-并发-JMM与线程同步)
    - AQS `state` 语义 + CLH 队列 + 模板方法 → [AQS 设计哲学](@java-并发-AQS设计哲学)
    - `ReentrantLock` / 线程池 7 参数 / `StampedLock` / `LongAdder` → [并发工具 Lock 与线程池](@java-并发-并发工具Lock与线程池)
    - `ConcurrentHashMap` 完整源码 / `CopyOnWriteArrayList` 弱一致性 → [并发集合与实战陷阱](@java-并发-并发集合与实战陷阱)
    - 虚拟线程 pin 到载体线程的排查 → [JVM 现代实践与前沿技术](@java-JVM-现代实践与前沿技术)
...
- `ConcurrentHashMap.put` **同时贯穿 `10a/10b/10c/10d` 四层**——空槽位 CAS 无锁插入（10a）、非空槽位 `synchronized` 头节点锁（10a）、扩容协助的 `sizeCtl` 语义借鉴 AQS `state`（10b）、整体 API 是并发数据结构（10d）。

这就是"战役三强耦合、不可跳读"的**根本原因**——一个 `HashMap.put` 就能把四层全用上，你没法通过"只学 10d"就搞懂它。
...
flowchart TD
    ...
    Warn["⚠️ 战役三强耦合<br/>不可跳读<br/>跳读的性能代价是<br/>后续所有'为什么'都没根"]
...
> *"并发编程之所以难，不是因为知识点多，是因为它**同时贯穿**'硬件事实 / JVM 语义 / 框架抽象 / 实战组合'四层——任何一层没打通，都会在另一层遇到看似神秘的 bug。战役三这四篇的强耦合动线，就是为了让你**一次性把这四层都打通**。"*
...
- **`10 → 10b`**：本页 §6 提到 `park` / `unpark` 家族与 `tryAcquire` / `tryRelease` 四大钩子——`10b` 需在"CLH 队列挂起唤醒"与"模板方法"两个章节各承接一次家族卡片，并解释"为什么 `unpark` 先于 `park` 调用不会丢信号"这个 CLH 队列的**关键硬件事实**。
- **`10 → 10c`**：本页 §6 提到 `*Adder` / `*Lock` / `*Handle` / `*Latch` 四大家族——`10c` 需**完整展开**这四大家族卡片，尤其 `*Adder` 家族的 `Striped64` 分段计数（这是 `LongAdder` 相比 `AtomicLong` 在高竞争下快数倍的**唯一根本原因**）。
- **`10 → 10d`**：本页 §3 提到 `ConcurrentHashMap.put` **同时贯穿四层**——`10d` 需在"完整源码"章节承接"CAS 无锁插入 + `synchronized` 头节点锁 + `sizeCtl` CAS 争抢 + `ForwardingNode` 协助扩容"这条**四层贯穿的底层链路**。
- **`10 → 12d`**：本页 §1 timeline 提到虚拟线程 pin 到载体线程的问题**不改变** AQS/CAS 底层机制——`12d` 需承接"虚拟线程 + `synchronized` = pin 现场；虚拟线程 + `ReentrantLock` = 不 pin"这条**JDK 21 关键差异**（`synchronized` 在虚拟线程中会导致载体线程被挂起、无法承载其他虚拟线程，而 `ReentrantLock` 基于 AQS 的 `LockSupport.park` 会正确让出载体线程）。
