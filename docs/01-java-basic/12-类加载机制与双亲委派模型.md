...
// 特例：final static 常量在准备阶段直接赋值为 456
...
│         加载 JDK 核心类库（rt.jar / java.base 模块）       │
│              由 C++ 实现，Java 中表示为 null               │
└───────────────────────────┬─────────────────────────────┘
                            │ parent
┌───────────────────────────▼─────────────────────────────┐
│           Extension ClassLoader（扩展类加载器）            │
│         加载 $JAVA_HOME/lib/ext 目录下的 JAR 包            │
│              Java 9+ 改名为 Platform ClassLoader          │
└───────────────────────────┬─────────────────────────────┘
                            │ parent
┌───────────────────────────▼─────────────────────────────┐
...
!!! note "Java 9 模块化改进"
    Java 9 引入模块系统后，SPI 的加载机制通过 `java.util.ServiceLoader` 进一步规范化，不再完全依赖线程上下文类加载器。
...
Bootstrap ClassLoader
    └── Extension ClassLoader
        └── Application ClassLoader（加载 Tomcat 自身）
            └── Common ClassLoader（Tomcat 公共类库）
                ├── Catalina ClassLoader（Tomcat 内部类）
                ├── Shared ClassLoader（多应用共享类）
                │   ├── WebApp1 ClassLoader（应用1，优先自加载）
                │   └── WebApp2 ClassLoader（应用2，优先自加载）
