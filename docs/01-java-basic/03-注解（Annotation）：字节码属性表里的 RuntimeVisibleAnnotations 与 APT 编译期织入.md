...
导致这个经典 Bug 的元凶，往往是因为系统引入了第三方框架（如某些旧版本的序列化工具、轻量级 AOP 织入工具或动态热部署插件）。它们在运行时出于某种原因动态修改或重新生成了类的代理，或者在配置注解时，开发者不小心混淆了注解的**生命周期边界**。

### 1.2 注解的“尸体”留在了哪里？

要破获上面的悬案，我们必须在脑海中建立起注解的“时空概念”。当我们在编辑器中敲下一个注解时，它其实开始了一段漫长的生命周期： `.java` 源码 -> `.class` 字节码 -> JVM 方法区内存。
...
注解在被写下之后，它的“尸体”到底留在了哪里？现在，让我们开启第二层，直接反编译类文件，抓出注解在字节码里的历史数据。

---

## 2. 字节码考古——属性表与动态代理

首先必须点透一个颠覆许多人认知的底层真相：**Java 的注解在 Class 二进制文件里没有任何可执行的代码指令。它仅仅是附着在类、方法、方法参数或字段上的一个“附加属性表（Attribute Table）”里的只读字符串元数据**。
...
由于 `RUNTIME` 注解深度绑定了反射机制，如果在核心高频调用的业务接口中高频触发注解的动态解析，系统就必须频繁付出反射带来的多重硬件代价（我们将在 1.5 篇中深挖反射慢的三大根本原因：安全检查、无法被 JIT 内联、参数装箱）。

认清了注解在“编译期降维固化”与“运行时反射伪造”这两大流派的性能代价后，我们才能在真实的工业级分布式架构中，画出正确的工程设计防线。
...
public Object intercept(ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    // 💥 底层痛点：每一次接口高频调用，都在强迫 JVM 去翻看属性表、调用反射
    RequiresRole annotation = method.getAnnotation(RequiresRole.class); 
