...
│  vtable（虚方法表）
│  ┌────────────────────────────────────────────────┐ 
│  │ [0] Object.toString()   → Object.toString 地址 
│  │ [1] Object.hashCode()   → Object.hashCode 地址
│  │ [2] Object.equals()     → Object.equals 地址
│  │ [3] Animal.eat()        → Dog.eat 地址（已重写）
│  │ [4] Animal.sleep()      → Animal.sleep 地址
│  │ [5] Dog.bark()          → Dog.bark 地址
│  └────────────────────────────────────────────────┘ 
│  静态变量、常量池、类元信息...
└────────────────────────────────────────────────────────┘
```
...
Dog 对象内存布局：
┌──────────────────────────────────────────────────────────────────┐
│  offset+0: Animal.name = "Animal"  ← animal.name 编译期绑定到这里
...
flowchart LR
    ...
    subgraph 接口
        IP["interface Drawable
            + void draw()
            + default void show()"]
        C2["Circle implements Drawable"]
        S2["SVGShape implements Drawable"]
        IP --> C2
        IP --> S2
    end
```
