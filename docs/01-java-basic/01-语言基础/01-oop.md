...
title: 面向对象（OOP） —— 封装、继承、多态与虚方法表
---

# 面向对象（OOP） —— 封装、继承、多态与虚方法表
...
3.4 节的 HashSet 重复计数陷阱，根源在于多态的动态分派：父类 `addAll()` 内部调用了 `add()`，即使发生在父类内部，JVM 也会执行子类重写后的 `add()`。下面拆解这一行代码在 JVM 底层的运转过程。
...
2. **初始化顺序的陷阱（对应 3.3 节）**：3.3 节给出的实例化顺序是**“先父类构造，后子类字段赋值”**。当父类构造方法执行到 `init()` 时，子类的 `name = "旺财"` 尚未执行；此时子类字段仍是默认值（即 `null`），问题就此发生。
