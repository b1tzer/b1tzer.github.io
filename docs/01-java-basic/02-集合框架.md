...
- **`Comparable` vs `Comparator`**：前者是对象自身的"自然排序"（实现接口），后者是外部传入的"定制排序"（策略模式）
...
每次扩容都会申请一块新的更大的内存，旧数组随即被废弃等待 GC 回收。如果新数组申请的地址恰好落在旧数组释放的地址范围内，操作系统就可以直接把那块物理内存页重新分配给新数组，而不必向内核申请全新的物理页——这就是**内存复用**。

内存复用的**核心关键**是：**新数组的大小 ≤ 之前所有废弃数组的大小之和**。只有满足这个条件，历史上释放的内存总量才足以容纳新数组，复用才有可能发生；反之，新数组比所有历史释放内存加起来还大，操作系统只能去申请全新的物理内存，产生更多内存碎片，GC 压力也更大。
...
> **结论**：`LinkedList` 不仅 CPU 缓存命中率低（内存不连续），内存占用也是 `ArrayList` 的 5 倍，**绝大多数场景应优先选 `ArrayList`**。
...
这是因为扩容后新增的那一位 bit 恰好是 `oldCapacity` 对应的位，只需判断这一位即可，避免了全量 rehash 的开销。
...
> **官方推荐**：用 `ArrayDeque` 替代 `Stack`（`Stack` 继承自 `Vector`，所有方法加锁，性能差）
...
> **注意**：写操作代价高（O(n) 复制），不适合写频繁的场景；读操作读的是快照，存在短暂的数据不一致（弱一致性）。
...
### ❌ 误区4：HashMap 的 key 未正确实现 hashCode 和 equals
