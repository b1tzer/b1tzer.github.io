...
ZGC 在 **64 位指针的高位** （HotSpot 中默认占 **bit 42~45**、共 4 位）直接编码 GC 状态，体现成“香色”。这 4 位分别表示：
...
示意图（并非精确位置，仅表示“指针高位 有几个状态标志 + 低 44 位存真实地址”）：
...
> 💡 早期资料常写“ZGC 最大支持 4TB”——那是 JDK 11~12 的早期限制。**JDK 13 起 ZGC 支持最大 16TB 堆**。
...
!!! info "📌 本章定位与边界声明"
    **本章属于外延阅读索引，与前 10 章的"深度源码型"骨架不同**，内容覆盖：① 容器化 JVM 调优（11.1）；② 虚拟线程（11.2）；③ 性能分析工具链（11.3）；④ 云原生最佳实践（11.4）；⑤ JIT 与内存屏障深度解析（11.5）；⑥ 生产故障案例库（11.6）；⑦ 未来趋势展望（11.7）。

    **风格说明**：本章以**实战要点清单 + 配置示例**为主，不追求每一小节都做到前 10 章同等深度的源码追溯。若读者关注的是"机制如何实现"，请优先阅读前 10 章对应主题。

    **后续拆分计划**：本章部分小节（尤其 11.5 JIT 与内存屏障、11.2 虚拟线程）的独立度已足以单独成文，后续将按 §6.3 拆为"**JVM 现代实践与前沿技术**"独立文档；彼时本章替换为一行 `📖 详见 @doc_id` 导引。
...
- **volatile的实现原理**：内存屏障插入，禁止指令重排序
- **final字段的内存语义**：构造器退出（return）之前对 final 字段的写入，对通过对象引用访问这些字段的其他线程均可见（无需 `synchronized` / `volatile`）——前提是 **this 引用未在构造期间逃逸**
...
JDK 21 引入 **分代 ZGC**（JEP 439），结合分代收集的理论优势和 ZGC 的低延迟特性：

- 新生代使用复制算法，快速回收短命对象
- 老年代使用 ZGC 的染色指针和并发转移
- 目标：更低延迟，更高吞吐量
- 启用方式：`-XX:+UseZGC -XX:+ZGenerational`（JDK 21）；JDK 23 起分代模式成为默认
