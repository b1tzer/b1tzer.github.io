...
> **一句话记忆口诀**：
>
> 1. **单点不可怕，可怕的是没切换方案**——高可用 = 监控 + 自动故障转移 + 数据冗余。
> 2. **MHA / Orchestrator / MGR / RDS 四选一**：按运维成本、一致性、切换时间三维度选型。
> 3. **读写分离必处理主从延迟**——强一致读走主库，写后读用 GTID 等待。
> 4. **GTID 是跨主从一致性读的通用抓手**——`WAIT_FOR_EXECUTED_GTID_SET` 比 SBM 可靠。
> 5. **连接池不是越大越好**——`(核数 × 2) + 磁盘数` 是经验公式，过多连接只会拖垮主库。
...
## 4. 半同步复制：`AFTER_SYNC` vs `AFTER_COMMIT`
