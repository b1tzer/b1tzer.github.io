...
!!! info "**数据类型与表设计规范 一句话口诀**"
    **类型选小不选大**：能用 `TINYINT` 不用 `INT`，能用 `INT` 不用 `BIGINT`

    **金额用 `DECIMAL` 或 `INT`（分）**，**绝对不用 `FLOAT` / `DOUBLE`**

    **字符集 `utf8mb4` + `utf8mb4_0900_ai_ci`** 是唯一正确答案

    **主键趋势递增**：单库自增、分布式雪花/UUIDv7，不用随机UUID

    **字段 `NOT NULL` + `DEFAULT` + `COMMENT`**，外键在应用层维护

> 📖 **边界声明**：本文聚焦"数据类型选择原则与表设计规范"，以下主题请见对应专题：
>
> - InnoDB存储引擎底层机制（Buffer Pool / Redo Log / 页分裂） → [InnoDB存储引擎深度剖析](@mysql-InnoDB存储引擎深度剖析)
> - 索引数据结构与优化策略（B+树 / 最左前缀 / 覆盖索引） → [索引详解](@mysql-索引详解)
> - 分库分表与分布式架构（水平拆分 / 分布式ID） → [分库分表与分布式架构](@mysql-分库分表与分布式架构)
> - 实战问题排查与调优（慢查询 / 死锁 / 主从延迟） → [实战问题与避坑指南](@mysql-实战问题与避坑指南)
...
**Q：为什么 UUID v4 做主键会导致大量页分裂？**

> 📖 **为什么 UUID v4 做主键会导致"大量页分裂"、而自增主键"索引紧凑"——底层是 InnoDB 聚簇索引 B+ 树的页结构与页分裂机制，详见 [InnoDB存储引擎深度剖析](@mysql-InnoDB存储引擎深度剖析)，本文不再展开。**

**Q：TEXT 类型为什么不能有默认值？**

> 📖 **底层限制题**：TEXT/BLOB 类型在 MySQL 存储引擎层有特殊处理，无法设置默认值。具体实现机制与溢出页管理详见 [InnoDB存储引擎深度剖析](@mysql-InnoDB存储引擎深度剖析) §4.1，本文专注使用规范。

**Q：大表 DELETE 操作如何避免锁争用？**

> 📖 **实战排查题**：大表 DELETE 会引发锁争用、Binlog 膨胀、主从延迟等问题。完整的线上排查 checklist 与分批删除方案详见 [实战问题与避坑指南](@mysql-实战问题与避坑指南) §坑8，本文专注表设计原则。
