...
> **一句话记忆口诀**：
>
> 1. **一致性**：写操作永远是"**更新 DB → 删除缓存**"（Cache Aside），**不是更新缓存**——并发写场景下更新会留脏数据；极端并发用**延迟双删**兜底，金融级用 **Canal 监听 Binlog** 彻底解耦。
> 2. **排行榜**用 `ZSet`，**计数器**用 `INCR`，**限流**用 `ZSet` 滑动窗口 / Redisson `RRateLimiter`，**消息队列**用 `Stream`（轻量）或直接选 Kafka（海量）。
> 3. **大 Key**：String > 10KB、集合 > 5000 元素，删除必须用 `UNLINK`（异步），**绝不能** `DEL`（阻塞主线程）。
> 4. **热 Key**：**本地缓存**（Caffeine）是第一选择，**Key 复制**（`key:replica:0~9`）分散到多副本是第二选择。
> 5. **选型铁律**：Redis 不是 MQ——Redis Stream 适合轻量消息（万级 QPS）；海量消息、磁盘堆积、消息回溯场景**必须**用 Kafka/RocketMQ。

> 📖 **边界声明**：本文聚焦"**Redis 在业务系统中的典型落地场景与疑难问题**"——一致性、限流、排行榜、大 Key/热 Key、消息队列、Session、GEO。以下相关主题请见对应专题：
>
> - **缓存穿透 / 击穿 / 雪崩的触发机制与防护方案** → [缓存三大问题](@redis-缓存三大问题)
> - **主从 / 哨兵 / 集群的高可用架构细节** → [高可用架构](@redis-高可用架构)
> - **分布式锁的底层实现（SETNX / Redisson / RedLock）** → [分布式锁](@redis-分布式锁)
> - **Lua 脚本的原子性原理** → [事务与 Lua 脚本](@redis-事务与Lua脚本)
> - **ZSet 底层跳表 + 字典、Stream 底层 Radix Tree 的数据结构** → [数据结构与底层编码](@redis-数据结构与底层编码)

!!! note "📖 术语家族：`缓存一致性模式族`"
    **字面义**：业务代码同时操作**缓存**和**数据库**两个存储时，协调二者状态保持一致的**读写模式家族**。
    **在 Redis 中的含义**：由于缓存与 DB 是两个独立存储、写操作非原子，**一致性模式的选择**直接决定了并发场景下的脏数据概率——选错模式（如"先更新缓存再更新 DB"）在并发下必出问题，选对模式（Cache Aside）在极端场景也只需一次延迟双删兜底。
    **同家族成员**：

    | 模式 | 写顺序 | 一致性 | 典型缺陷 | 推荐度 |
    | :-- | :-- | :-- | :-- | :-- |
    | `Cache Aside`（旁路缓存）| **更新 DB → 删缓存** | 较高 | 极端并发读写下短暂不一致 | ✅ **主流首选** |
    | `Read/Write Through` | 缓存代理读写 DB | 高 | 需缓存层具备 DB 直连能力 | ⚠️ 依赖框架（EhCache）|
    | `Write Behind`（异步回写）| 只写缓存，异步刷 DB | 低（可能丢数据）| 缓存宕机数据丢失 | ⚠️ 高性能但不安全 |
    | `延迟双删` | Cache Aside + 延迟再删一次 | 更高 | 延迟时长难定 | ✅ Cache Aside 兜底 |
    | `Canal 同步` | 监听 Binlog 异步同步 | 最终一致（秒级）| 部署 Canal 复杂度高 | ✅ 多下游场景首选 |

    **命名规律**：**Aside / Through / Behind** 三个介词定位了"应用代码与缓存的相对位置"——Aside = **旁路**（应用自己管缓存），Through = **穿透**（应用写缓存、缓存代写 DB），Behind = **滞后**（应用写缓存、缓存异步回写 DB）。记住介词的方位含义，家族成员的行为模式立刻清晰。
