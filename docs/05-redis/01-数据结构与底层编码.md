...
**类比**：String 就像**保险箱里的一个格子**——一个钥匙（key）对应一个格子（value），格子里可以放任何东西：现金（整数）、文件（字符串）、珠宝（二进制数据）。

**为什么需要 String？**

Redis 作为内存数据库，最基本的需求就是**存储单个值**。String 的设计哲学是"**简单至上**"：

- **最基础**：所有其他数据类型都基于 String 构建
- **最通用**：能存储任何可序列化的数据
- **最高效**：单值操作天然适合缓存、计数器等高频场景

**核心原理：从用户视角到内部实现**

**用户看到的**：`key → value` 的简单映射
...
| 编码 | 触发条件 | 设计原理 | 内存特点 | 时间复杂度 |
| :-- | :-- | :-- | :-- | :-- |
| `int` | 值为整数且在 long 范围内 | **直接存二进制整数**，避免字符串转换开销 | 最省内存（8 字节） | O(1) |
| `embstr` | 字符串长度 ≤ 44 字节 | **对象头 + SDS 连续分配**，一次内存操作 | 缓存友好（同一缓存行） | O(1) |
| `raw` | 字符串长度 > 44 字节 | **对象头 + SDS 分开分配**，避免大对象拖累 | 内存占用稍多 | O(1) |

> ⭐ **为什么是 44 字节？** RedisObject（16 字节）+ sdshdr8（3 字节）+ `\0`（1 字节）= 20 字节，64 字节缓存行 - 20 = 44 字节可用空间。
...
> ⚠️ **避坑**：不要用 String 存整个 JSON 对象来模拟对象存储——更新时需要全量覆盖，并发场景下会互相覆盖。应该用 Hash 存储对象字段（见 3.2 节）。

**一句话口诀**：String = 单值存储 + 原子操作 + 自动编码，**简单场景的首选，复杂场景的基础**。
...
**查找过程**：从最高层开始，在当前层**向右移动直到找到第一个大于等于目标值的节点**，然后向下一层继续，重复此过程直到找到目标节点或到达最底层。

**算法细节**：

- 每层只访问部分节点，不是遍历全层
- 从高层到低层，跳跃式前进，平均时间复杂度 O(log n)
- 跳表通过随机层数实现平衡，平均层数 ≈ log₂n

### 4.2 随机层数：跳表的灵魂机制

**随机层数**是跳表相比传统平衡树的核心创新，它用**概率平衡**替代了复杂的旋转操作。

#### 为什么需要随机层数？

传统平衡树（红黑树、AVL树）需要复杂的旋转/变色操作来维持精确平衡，实现复杂且维护成本高。跳表放弃了"精确平衡"，采用"**概率平衡**"：

- 不保证绝对平衡，但**数学期望**上能达到 O(log n) 性能
- 通过随机性来"大概率"保持平衡
- 实现简单，代码量少（Redis 跳表实现仅几十行）
...
int randomLevel() {
    int level = 1;
    // 每次有 1/4 的概率增加一层（Redis 实际使用 0.25）
...
**平均层数**：E(L) = 1 + 1/4 + 1/16 + ... ≈ 1.33 层

**最高层期望**：E(max_level) ≈ log₄n

- 100个节点时，最高层期望 ≈ log₄100 ≈ 3.3层
- 100万个节点时，最高层期望 ≈ log₄1000000 ≈ 10层

**查找性能保证**：由于层数期望是 O(log n)，查找路径长度也是 O(log n)。

#### Redis 的实际实现优化

Redis 7.0 的跳表实现（`t_zset.c`）中：

- 增长概率从经典的 50% 调整为 **25%**（`ZSKIPLIST_P = 0.25`）
- 节点更稀疏，内存更节省
- 仍然保证 O(log n) 性能
- 最大层数限制为 32 层（`ZSKIPLIST_MAXLEVEL = 32`）

#### 为什么随机层数能工作？

**核心原理**：大数定律

虽然单个节点的层数是随机的，但当节点数量足够大时：

- 高层节点的分布会趋于均匀
- 查找路径的长度会稳定在 O(log n)
- 极端不平衡的情况概率极低（可忽略不计）

**工程实践证明**：在实际应用中，跳表的性能与红黑树相当，甚至在某些场景下更优，而实现复杂度大大降低。
