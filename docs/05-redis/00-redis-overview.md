...
timeline
    title Redis 版本发展
    2009 : Redis 1.0 : 五种基本数据结构、单线程模型
    2012 : Redis 2.6 : Lua 脚本、PEXPIRE、位图操作
    2013 : Redis 2.8 : Sentinel 哨兵、SCAN 系列、PSYNC 部分同步
    2015 : Redis 3.0 : Redis Cluster 集群正式版
    2017 : Redis 4.0 : 混合持久化、UNLINK 异步删除、Modules 模块系统、LFU 淘汰
    2018 : Redis 5.0 : Stream 数据类型、有序集合弹出操作
    2020 : Redis 6.0 : 多线程网络 IO、ACL 权限控制、RESP3、客户端缓存
    2022 : Redis 7.0 : Redis Functions、ACL v2、分片 Pub/Sub、MP-AOF
    2024 : Redis 7.2 : 哨兵 TLS、listpack 全面替代 ziplist
```

> 📌 本站聚焦 **Redis 6.0 ~ 7.2**。6.0 起支持多线程网络 IO（命令执行仍单线程），7.0 引入 Functions 和 MP-AOF，7.2 彻底用 listpack 替代 ziplist。自 7.4 起 Redis 切换为 RSALv2 + SSPLv1 双协议，社区出现 Valkey 等开源分支。
...
| 02 | **指令解析与执行机制** | RESP 协议解析 → 命令查找 → 单线程执行 → 返回结果，理解 Redis 为什么快 | [指令解析与执行机制](@redis-指令解析与执行机制) |
| 03 | **持久化机制** | RDB 快照恢复快但可能丢数据，AOF 安全但文件大，混合持久化兼顾两者 | [持久化机制RDB与AOF](@redis-持久化机制RDB与AOF) |
| 04 | **缓存三大问题** | 穿透用布隆过滤器，击穿用互斥锁/逻辑过期，雪崩加随机 TTL | [缓存三大问题](@redis-缓存三大问题) |
| 05 | **高可用架构** | 主从复制做读写分离，哨兵做自动故障转移，集群做水平扩展 | [高可用架构](@redis-高可用架构) |
| 06 | **分布式锁** | SETNX 有缺陷，Redisson 看门狗自动续期，RedLock 多节点容错 | [分布式锁](@redis-分布式锁) |
| 07 | **应用型问题** | 缓存一致性用 Cache Aside/延迟双删/Canal，大 Key 要拆分 | [应用型问题](@redis-应用型问题) |
| 08 | **内存管理与淘汰机制** | 惰性+定期删除过期 Key，allkeys-lru 最常用淘汰策略 | [内存管理与淘汰机制](@redis-内存管理与淘汰机制) |
| 09 | **事务与 Lua 脚本** | MULTI/EXEC 事务不支持回滚，Lua 脚本保证原子性 | [事务与Lua脚本](@redis-事务与Lua脚本) |
| 10 | **单线程模型与网络 IO** | 单线程避免锁竞争，epoll 多路复用处理高并发连接 | [单线程模型与网络IO](@redis-单线程模型与网络IO) |

---

## Redis 命名哲学：13 个家族一张图

Redis 的类名 / 配置 / 命令看似零散，但背后有一套**强一致的命名规律**——读懂词根，就能从命令名猜出行为、从配置项猜出生效位置。本站已为 13 个核心家族建立术语卡片（各源头文档都有完整展开，登记在 `.codebuddy/plan/terminology-glossary.md`），本节按"**动词 / 名词 / 修饰词**"三类做顶层俯瞰。
...
| 家族 | 词根含义 | 源头文档 | 一句话识别规律 |
| :-- | :-- | :-- | :-- |
| **指令解析族** | 命令从网络到执行的完整流程 | [指令解析与执行机制](@redis-指令解析与执行机制) | `RESP` 协议解析 → `Command Table` 查找 → `Hook` 钩子执行 → `Single Thread` 原子执行 |
...
>
> 1. **命令 = 类型首字母 + 动作**：`H`SET/`L`PUSH/`S`ADD/`Z`RANGE——第一个字母是 `Hash/List/Set/ZSet` 首字母。String 属于最基础类型，默认缺省就是指 String 类型。
...
| Redis 命令是如何解析和执行的？ | [指令解析与执行机制](@redis-指令解析与执行机制) |
