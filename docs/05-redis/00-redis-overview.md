...
| 07 | [内存管理与淘汰机制](./07-内存管理与淘汰机制.md) | jemalloc 内存分配器、内存碎片整理、Key 过期删除（惰性+定期）、8 种淘汰策略、LRU vs LFU 原理 |
| 08 | [事务与 Lua 脚本](./08-事务与Lua脚本.md) | MULTI/EXEC 事务局限性、WATCH 乐观锁、Lua 脚本原子性、Pipeline vs 事务 vs Lua 选型 |
| 09 | [单线程模型与网络 IO](./09-单线程模型与网络IO.md) | 单线程为什么快、epoll IO 多路复用、Reactor 模型、Redis 6.0 多线程网络 IO、慢查询排查 |
