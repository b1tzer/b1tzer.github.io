...
[⬅️ 上一篇：Redis 持久化机制：RDB 与 AOF](02-持久化机制RDB与AOF.md) | [🏠 返回目录](../README.md) | [下一篇：Redis 高可用架构：主从、哨兵、集群 ➡️](04-高可用架构.md)
...
flowchart TD
    ...
    subgraph 缓存雪崩 - 大量 Key 同时过期
        A3["大量 Key\n设置相同 TTL"] --> B3["同一时刻集体过期"]
        B3 --> C3["大量请求同时穿透\n→ DB 崩溃"]
        C3 --> D3["解决方案:\n① TTL 加随机值\n② 多级缓存\n③ 熔断降级"]
    end
```
